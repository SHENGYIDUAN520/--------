...
def wifi_connect(ssid, pwd, retries=3, timeout=10):
    """标准WiFi连接实现[^1][^2]"""
    ...
    for wifi in wlan.scan():
        print(f"- {wifi.ssid} (强度:{wifi.rssi}dBm)")
    # 连接逻辑
...
if __name__ == "__main__":
    ...
    # 新增视频流配置
    streamer = VideoStreamer("192.168.131.19", 5000)  # 目标服务器IP
    if not streamer.connect():
        sys.exit("流媒体服务器连接失败")    
    
    if not wifi_connect(WIFI_SSID, WIFI_PASSWORD, retries=5, timeout=15):
        sys.exit("网络初始化失败")
    ...
    while True:
        with ScopedTiming("total",1):
            ...
            # 转换为HWC格式并发送[^6]
            img_hwc = np.squeeze(img)                   # 从NCHW转为HWC
