...
- `device_id`: 设备唯一标识
- `server_ip`: 服务器IP地址，本地开发使用localhost，部署时使用实际服务器地址
- `server_port`: API服务端口
- `video_port`: 视频流传输端口
- `reconnect_interval`: 连接断开后重试间隔(秒)
- `camera`: 摄像头配置
  - `width`: 图像宽度(像素)
  - `height`: 图像高度(像素)
  - `fps`: 帧率
- `ai`: AI功能配置
  - `enable`: 是否启用AI功能
  - `fall_detection`: 是否启用跌倒检测
  - `pose_detection`: 是否启用姿态检测
  - `face_recognition`: 是否启用人脸识别
